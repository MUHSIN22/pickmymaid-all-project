...
const Cards = () => {
  ...
  const { country, keyword } = useParams<{
    country?: string;
    keyword?: string;
  }>();
  ...
  useEffect(() => {
    ...
    if (country && keyword) {
      newQuery += `country=${countries[country]}&service=${
        services[filterSearchParams(keyword as string)[0]]
      }`;
      ...
      const filteredKeyword = filterSearchParams(keyword as string);
      newQuery += `country=${countries[filteredKeyword?.[1]]}&service=${
        services[filteredKeyword?.[0]]
      }`;
    }
        ...
        {maids &&
          maids[currentPage]?.map((item, i) => (
            <MaidCard
              key={i}
              {...item}
              experience={item.employmentHistory || 0}
            />
          ))}
      </div>
    </div>
