      ...
      <DropdownMenu.Trigger asChild>
        <button
          className={`px-4 bg-white py-2 ${
            smallDevice ? "rounded-[0px_0px_10px_0px]" : "rounded-[10px_0px]"
          } border-[1px] outline-0 border-primary-400 text-primary-400 flex gap-1 items-center`}
        >
          Sort{" "}
          {query ? (
            <span className="opacity-80 whitespace-nowrap">{query}</span>
          ) : (
            ""
          )}
          <RiArrowUpDownLinel />
