...
} from "../../../lib/features/maid/maidSlice";
...
const SearchMaid = () => {
  ...
  useEffect(() => {
    if (statusHeading?.current) {
      localStorage.setItem(
        "cardTop",
        statusHeading?.current?.getBoundingClientRect()?.top.toString()
      );
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    if (URLCountry && keyword) {
      setSelectedData(
        `${filterSearchParams(keyword as string)[0]}_${URLCountry}`
      );
          ...
          <div className="mt-[30px] justify-between gap-[10px] flex flex-col md:flex-row sm:flex-col">
            ...
            >
              ...
              <div className="md:block hidden">
                <SortMenu items={sorting} onSort={sortHandler} />
              </div>
            </div>
            <div className="flex md:hidden text-[16px] justify-center items-center fixed bottom-[85px] z-10 md:left-0 left-1/2 translate-x-[-50%]">
              <div className="flex">
                ...
                <SortMenu smallDevice items={sorting} onSort={sortHandler} />
              </div>
            </div>
          ...
          <div className="grid place-items-center">
            <Pagination
              ...
              onPageChange={(page) => {
                ...
                window.scrollTo(
                  0,
                  Number(localStorage.getItem("cardTop")) || 0
                );
              }}
